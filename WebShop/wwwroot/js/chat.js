...
function deleteRoom() {
    ...
    $.ajax({
        url: `/ChatRooms/DeleteChatRoom/${roomId}`, //используем интерполяцию строк чтобы использовать переменную
        dataType: "json",
        type: "DELETE",
        ...
        success: function (json) {
            ...
            connection.send("SendDeleteRoomMessage", json.deleted, json.selected, roomName); //сейчас selected не использу, для будущего это
            fillRoomDropDown();
        },
        error: function (xhr) {
            alert('error');
