...
connectionChat.on("MessageReceived", function (sender, message, reciever) {
    var li = document.createElement("li");
    document.getElementById("messagesList").appendChild(li);
    li.textContent = `[${sender}] : ${message} to -> ${reciever}`;
});
...
document.getElementById("sendMessage").addEventListener("click", function (event) {
    ...
    if (receiver.length > 0) {
        ...
        connectionChat.send("SendMessageToAll", sender, message).catch(function (err) {
            return console.error(err.error(err.toString()));
        });
    }
